...
После 4 пункта (Реализация системы) пишем раздел **5. Индивидуальное задание (Вариант 19)**.

Вот продолжение в твоём формате:

---

## 5. Индивидуальное задание (Вариант 19)

**Условие:** Предварительная авторизация — сервер выдает токены доступа. Клиент сначала получает токен, потом делает запросы.

### Реализованные требования

| Требование | Реализация |
|------------|------------|
| Эндпоинт `/login` для выдачи токена | `secrets.token_hex(16)` |
| Эндпоинт `/api/data` с проверкой токена | проверка из заголовка `Authorization` |
| Токен передаётся перед каждым запросом | в заголовке `Authorization` |
| Отказоустойчивость сохранена | координатор переключает серверы |

### Схема работы авторизации

```mermaid
flowchart LR
    subgraph A[1. Получение токена]
        C1[Клиент] -->|POST /login| L[Координатор]
        L -->|POST /login| S[Сервер]
        S -->|token| L
        L -->|token| C1
    end
    
    subgraph B[2. Запрос с токеном]
        C2[Клиент] -->|POST /api/data<br/>Header: Authorization: token| L2[Координатор]
        L2 -->|запрос с токеном| S2[Сервер]
        S2 -->|ответ| L2
        L2 -->|ответ| C2
    end
```
...
**Клиент (использование токена):**

```python
# Получение токена
token = requests.post("http://127.0.0.1:8000/login", json={}).json()["token"]
...
```
✅ Токен получен: 9ca8047daface716dc817efac928eb49
✅ Ответ сервера: {"status": "ok", "message": "Processed: Привет, мир!"}
```

**Лог сервера при получении токена:**
...
**Лог сервера при проверке токена:**
...
**Лог сервера при невалидном токене:**

```
❌ Отказ доступа: невалидный токен
```
...
| Терминал | Команда |
|----------|---------|
| 1 | `python3 server.py 5001` |
| 2 | `python3 server.py 5002` |
| 3 | `python3 coordinator.py` |
| 4 | `python3 client.py` |
...
|-----|----------|-----------|
| 1 | Оба сервера запущены | Запрос уходит на Сервер 1 (5001) |
| 2 | Сервер 1 остановлен (Ctrl+C) | Координатор фиксирует ошибку |
| 3 | Повторный запрос от клиента | Координатор переключает на Сервер 2 (5002) |
| 4 | Клиент получает ответ | Успешный ответ от Сервера 2 |

**Лог координатора при отказе:**
...
**Вывод клиента (после переключения):**
...
```
✓ Токен валиден: 9ca8047daface716dc817efac928eb49
✓ Данные расшифрованы: Проверка failover
```
...
| Сценарий | Статус |
|----------|--------|
| Получение токена через `/login` | ✅ |
| Шифрование данных Fernet на клиенте | ✅ |
| Расшифровка Fernet на сервере | ✅ |
| Проверка токена перед обработкой | ✅ |
| Отказоустойчивость (5001 → 5002) | ✅ |
| Проксирование заголовка Authorization | ✅ |
| mTLS между координатором и сервером | ✅ |

---

## 8. Заключение

В ходе выполнения лабораторной работы разработана распределенная система, обеспечивающая:

- **mTLS** — взаимную аутентификацию сервера и клиента
- **Fernet** — симметричное шифрование данных (AES-128-CBC + HMAC-SHA256)
- **Токены доступа** — предварительную авторизацию (Вариант 19)
- **Failover** — автоматическое переключение при отказе сервера (5001 → 5002)
...
Все требования лабораторной работы выполнены. Система работает стабильно и готова к демонстрации.
...
├── coordinator.py            # Координатор (балансировщик)
...
├── ca_cert.pem               # Корневой сертификат CA
├── ca_key.pem                # Приватный ключ CA
